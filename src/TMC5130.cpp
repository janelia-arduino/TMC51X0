...
#ifdef ESP32
void TMC5130::setup(HardwareSerial & serial,
...
#if SOFTWARE_SERIAL_IMPLEMENTED
void TMC5130::setup(SoftwareSerial & serial,
  long serial_baud_rate,
  SerialAddress serial_address)
{
...
void TMC5130::setHardwareEnablePin(uint8_t hardware_enable_pin)
{
...
void TMC5130::setMicrostepsPerStep(uint16_t microsteps_per_step)
{
...
void TMC5130::setMicrostepsPerStepPowerOfTwo(uint8_t exponent)
{
  switch (exponent)
  {
    case 0:
...
void TMC5130::setRunCurrent(uint8_t percent)
{
...
void TMC5130::setHoldCurrent(uint8_t percent)
{
...
void TMC5130::setHoldDelay(uint8_t percent)
{
...
void TMC5130::setAllCurrentValues(uint8_t run_current_percent,
...
void TMC5130::enableInverseMotorDirection()
{
  global_config_.shaft = 1;
  writeStoredGlobalConfig();
}

void TMC5130::disableInverseMotorDirection()
{
  global_config_.shaft = 0;
  writeStoredGlobalConfig();
}

void TMC5130::setStandstillMode(TMC5130::StandstillMode mode)
{
  pwm_config_.freewheel = mode;
  writeStoredPwmConfig();
}

void TMC5130::enableAutomaticCurrentScaling()
{
  pwm_config_.pwm_autoscale = STEPPER_DRIVER_FEATURE_ON;
  writeStoredPwmConfig();
}

void TMC5130::disableAutomaticCurrentScaling()
{
  pwm_config_.pwm_autoscale = STEPPER_DRIVER_FEATURE_OFF;
  writeStoredPwmConfig();
}

void TMC5130::enableAutomaticGradientAdaptation()
{
  pwm_config_.pwm_autograd = STEPPER_DRIVER_FEATURE_ON;
  writeStoredPwmConfig();
}

void TMC5130::disableAutomaticGradientAdaptation()
{
  pwm_config_.pwm_autograd = STEPPER_DRIVER_FEATURE_OFF;
  writeStoredPwmConfig();
}

void TMC5130::setPwmOffset(uint8_t pwm_amplitude)
{
  pwm_config_.pwm_offset = pwm_amplitude;
  writeStoredPwmConfig();
}

void TMC5130::setPwmGradient(uint8_t pwm_amplitude)
{
  pwm_config_.pwm_grad = pwm_amplitude;
  writeStoredPwmConfig();
}

void TMC5130::setPowerDownDelay(uint8_t power_down_delay)
{
  write(ADDRESS_TPOWERDOWN, power_down_delay);
}

void TMC5130::setReplyDelay(uint8_t reply_delay)
{
...
void TMC5130::moveAtVelocity(int32_t microsteps_per_period)
{
  write(ADDRESS_VACTUAL, microsteps_per_period);
}

void TMC5130::moveUsingStepDirInterface()
{
  write(ADDRESS_VACTUAL, VACTUAL_STEP_DIR_INTERFACE);
}

void TMC5130::enableStealthChop()
{
  global_config_.enable_spread_cycle = 0;
  writeStoredGlobalConfig();
}

void TMC5130::disableStealthChop()
{
  global_config_.enable_spread_cycle = 1;
  writeStoredGlobalConfig();
}

void TMC5130::setCoolStepDurationThreshold(uint32_t duration_threshold)
{
  write(ADDRESS_TCOOLTHRS, duration_threshold);
}

void TMC5130::setStealthChopDurationThreshold(uint32_t duration_threshold)
{
  write(ADDRESS_TPWMTHRS, duration_threshold);
}

void TMC5130::setStallGuardThreshold(uint8_t stall_guard_threshold)
{
  write(ADDRESS_SGTHRS, stall_guard_threshold);
}

void TMC5130::enableCoolStep(uint8_t lower_threshold,
...
void TMC5130::setCoolStepCurrentIncrement(CurrentIncrement current_increment)
{
  cool_config_.seup = current_increment;
  write(ADDRESS_COOLCONF, cool_config_.bytes);
}

void TMC5130::setCoolStepMeasurementCount(MeasurementCount measurement_count)
{
  cool_config_.sedn = measurement_count;
  write(ADDRESS_COOLCONF, cool_config_.bytes);
}

void TMC5130::enableAnalogCurrentScaling()
{
  global_config_.i_scale_analog = 1;
  writeStoredGlobalConfig();
}

void TMC5130::disableAnalogCurrentScaling()
{
  global_config_.i_scale_analog = 0;
  writeStoredGlobalConfig();
}

void TMC5130::useExternalSenseResistors()
{
  global_config_.internal_rsense = 0;
  writeStoredGlobalConfig();
}

void TMC5130::useInternalSenseResistors()
{
...
bool TMC5130::isCommunicating()
{
  return (getVersion() == VERSION);
}

bool TMC5130::isSetupAndCommunicating()
{
  return serialOperationMode();
}

bool TMC5130::isCommunicatingButNotSetup()
{
  return (isCommunicating() && (not isSetupAndCommunicating()));
}

bool TMC5130::hardwareDisabled()
{
  Input input;
  input.bytes = read(ADDRESS_IOIN);
...
uint16_t TMC5130::getMicrostepsPerStep()
{
...
TMC5130::Settings TMC5130::getSettings()
{
  Settings settings;
...
TMC5130::Status TMC5130::getStatus()
{
...
uint8_t TMC5130::getInterfaceTransmissionCounter()
{
  return read(ADDRESS_IFCNT);
}

uint32_t TMC5130::getInterstepDuration()
{
  return read(ADDRESS_TSTEP);
}

uint16_t TMC5130::getStallGuardResult()
{
  return read(ADDRESS_SG_RESULT);
}

uint8_t TMC5130::getPwmScaleSum()
{
  PwmScale pwm_scale;
...
int16_t TMC5130::getPwmScaleAuto()
{
  PwmScale pwm_scale;
  pwm_scale.bytes = read(ADDRESS_PWM_SCALE);
...
uint8_t TMC5130::getPwmOffsetAuto()
{
  PwmAuto pwm_auto;
...
uint8_t TMC5130::getPwmGradientAuto()
{
  PwmAuto pwm_auto;
  pwm_auto.bytes = read(ADDRESS_PWM_AUTO);
...
uint16_t TMC5130::getMicrostepCounter()
{
  return read(ADDRESS_MSCNT);
}

// private
void TMC5130::initialize(long serial_baud_rate,
  SerialAddress serial_address)
{
...
size_t TMC5130::serialWrite(uint8_t c)
{
  if (hardware_serial_ptr_ != nullptr)
  {
...
void TMC5130::setOperationModeToSerial(SerialAddress serial_address)
{
...
void TMC5130::setRegistersToDefaults()
{
...
void TMC5130::readAndStoreRegisters()
{
...
bool TMC5130::serialOperationMode()
{
...
void TMC5130::minimizeMotorCurrent()
{
...
uint32_t TMC5130::reverseData(uint32_t data)
{
...
uint8_t TMC5130::calculateCrc(Datagram & datagram,
...
template<typename Datagram>
void TMC5130::sendDatagramUnidirectional(Datagram & datagram,
  uint8_t datagram_size)
{
  uint8_t byte;
...
template<typename Datagram>
void TMC5130::sendDatagramBidirectional(Datagram & datagram,
  uint8_t datagram_size)
{
  uint8_t byte;
...
void TMC5130::write(uint8_t register_address,
  uint32_t data)
{
...
uint32_t TMC5130::read(uint8_t register_address)
{
...
uint8_t TMC5130::percentToCurrentSetting(uint8_t percent)
{
...
uint8_t TMC5130::currentSettingToPercent(uint8_t current_setting)
{
...
uint8_t TMC5130::percentToHoldDelaySetting(uint8_t percent)
{
  uint8_t constrained_percent = constrain_(percent,
    PERCENT_MIN,
    PERCENT_MAX);
...
uint8_t TMC5130::holdDelaySettingToPercent(uint8_t hold_delay_setting)
{
...
void TMC5130::writeStoredGlobalConfig()
{
  write(ADDRESS_GCONF, global_config_.bytes);
}

uint32_t TMC5130::readGlobalConfigBytes()
{
  return read(ADDRESS_GCONF);
}

void TMC5130::writeStoredDriverCurrent()
{
  write(ADDRESS_IHOLD_IRUN, driver_current_.bytes);
...
void TMC5130::writeStoredChopperConfig()
{
  write(ADDRESS_CHOPCONF, chopper_config_.bytes);
}

uint32_t TMC5130::readChopperConfigBytes()
{
  return read(ADDRESS_CHOPCONF);
}

void TMC5130::writeStoredPwmConfig()
{
  write(ADDRESS_PWMCONF, pwm_config_.bytes);
}

uint32_t TMC5130::readPwmConfigBytes()
{
  return read(ADDRESS_PWMCONF);
}

uint32_t TMC5130::constrain_(uint32_t value, uint32_t low, uint32_t high)
{
