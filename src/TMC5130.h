...
public:
  ...
  #else
  // Identify which microcontroller serial port is connected to the TMC5130 e.g.
  // Serial1, Serial2, etc. Optionally identify which serial address is assigned
  // to the TMC5130 if not the default of SERIAL_ADDRESS_0.
  void setup(HardwareSerial & serial,
    long serial_baud_rate=115200,
    SerialAddress serial_address=SERIAL_ADDRESS_0);
  #endif
  ...
  // check to make sure TMC5130 is properly setup and communicating
