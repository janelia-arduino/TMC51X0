...
public:
  ...
  enum InterfaceMode { SpiMode, UartMode };
  ...
  // error model.
  virtual Result<void> writeRegisterResult(uint8_t register_address,
                                           uint32_t data) {
    writeRegister(register_address, data);
    return Result<void>{};
  }

  virtual Result<uint32_t> readRegisterResult(uint8_t register_address) {
    Result<uint32_t> r;
