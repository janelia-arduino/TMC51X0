...
tmc51x0::Registers::DeviceModel TMC51X0::deviceModel() const {
  return registers.deviceModel();
}

bool TMC51X0::resyncReadableConfiguration() {
...
bool TMC51X0::updateDeviceModelFromVersion_(uint8_t version) {
...
bool TMC51X0::finishSetupOrRecovery_() {
  registers.notePossibleDrift();
...
void TMC51X0::resetHomeTracking_() {
  home_succeeded_ = false;
  home_failed_ = false;
}
