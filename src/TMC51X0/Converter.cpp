...
DriverParameters
Converter::driverParametersRealToChip(DriverParameters parameters) {
  DriverParameters parameters_chip = parameters;
  parameters_chip.global_current_scaler =
      percentToGlobalCurrentScaler(parameters.global_current_scaler);
  parameters_chip.run_current = percentToCurrentSetting(parameters.run_current);
  parameters_chip.hold_current =
      percentToCurrentSetting(parameters.hold_current);
  ...
  parameters_chip.stealth_chop_threshold =
      velocityRealToTstep(parameters.stealth_chop_threshold);
  parameters_chip.cool_step_threshold =
      velocityRealToTstep(parameters.cool_step_threshold);
  parameters_chip.high_velocity_threshold =
      velocityRealToTstep(parameters.high_velocity_threshold);

  return parameters_chip;
}

ControllerParameters
Converter::controllerParametersRealToChip(ControllerParameters parameters) {
  ControllerParameters parameters_chip = parameters;
  parameters_chip.max_velocity = velocityRealToChip(parameters.max_velocity);
  parameters_chip.max_acceleration =
      accelerationRealToChip(parameters.max_acceleration);
  parameters_chip.start_velocity =
      velocityRealToChip(parameters.start_velocity);
  parameters_chip.stop_velocity = velocityRealToChip(parameters.stop_velocity);
  parameters_chip.first_velocity =
      velocityRealToChip(parameters.first_velocity);
  parameters_chip.first_acceleration =
      accelerationRealToChip(parameters.first_acceleration);
  parameters_chip.max_deceleration =
      accelerationRealToChip(parameters.max_deceleration);
  parameters_chip.first_deceleration =
      accelerationRealToChip(parameters.first_deceleration);
  parameters_chip.zero_wait_duration =
      millisecondsToZeroWaitDuration(parameters.zero_wait_duration);
  parameters_chip.min_dc_step_velocity =
      velocityRealToChip(parameters.min_dc_step_velocity);
...
HomeParameters Converter::homeParametersRealToChip(HomeParameters parameters) {
  ...
  parameters_chip.hold_current =
      percentToCurrentSetting(parameters.hold_current);
  parameters_chip.target_position =
      positionRealToChip(parameters.target_position);
  parameters_chip.velocity = velocityRealToChip(parameters.velocity);
  parameters_chip.acceleration =
      accelerationRealToChip(parameters.acceleration);
  parameters_chip.zero_wait_duration =
      millisecondsToZeroWaitDuration(parameters.zero_wait_duration);
...
StallParameters
Converter::stallParametersRealToChip(StallParameters parameters) {
...
int32_t Converter::positionChipToReal(int32_t position_chip) {
  return position_chip /
         (int32_t)converter_parameters_.microsteps_per_real_position_unit;
}
...
int32_t Converter::velocityFloatToChip(float velocity_real) {
  float velocity_hz =
      velocity_real *
      (float)converter_parameters_.microsteps_per_real_position_unit /
...
uint8_t Converter::percentToGlobalCurrentScaler(uint8_t percent) {
  ...
  uint16_t scaler = map(constrained_percent, PERCENT_MIN, PERCENT_MAX,
                        GLOBAL_SCALER_MIN, GLOBAL_SCALER_MAX);
...
uint8_t Converter::percentToPwmSetting(uint8_t percent) {
  uint8_t constrained_percent = constrain_(percent, PERCENT_MIN, PERCENT_MAX);
  uint8_t pwm_setting = map(constrained_percent, PERCENT_MIN, PERCENT_MAX,
                            PWM_SETTING_MIN, PWM_SETTING_MAX);
  return pwm_setting;
}

uint32_t Converter::millisecondsToZeroWaitDuration(uint32_t milliseconds) {
  uint8_t clock_duration_ns =
      CLOCK_FREQUENCY_TO_DURATION_SCALER /
      (uint16_t)converter_parameters_.clock_frequency_mhz;
  return (milliseconds * MILLISECONDS_PER_SECOND) /
         (TZEROWAIT_SCALER * clock_duration_ns);
}
...
int32_t Converter::velocityChipToHz(int32_t velocity_chip) {
  int64_t velocity_hz;
  velocity_hz = (int64_t)velocity_chip *
                ((int64_t)converter_parameters_.clock_frequency_mhz * 1000000);
...
int32_t Converter::velocityRealToHz(int32_t velocity_real) {
  ...
  velocity_hz = velocity_hz /
                (int32_t)converter_parameters_.seconds_per_real_velocity_unit;
  return velocity_hz;
}

int32_t Converter::velocityHzToReal(int32_t velocity_hz) {
  int32_t velocity_real;
  velocity_real = velocity_hz *
                  (int32_t)converter_parameters_.seconds_per_real_velocity_unit;
...
int32_t Converter::tstepToVelocityHz(int32_t tstep) {
  ...
  int64_t velocity_hz;
  velocity_hz =
      ((int64_t)converter_parameters_.clock_frequency_mhz * 1000000) / tstep;
  return velocity_hz;
}
...
int32_t Converter::velocityHzToTstep(int32_t velocity_hz) {
  ...
  int64_t tstep;
  tstep = ((int64_t)converter_parameters_.clock_frequency_mhz * 1000000) /
          velocity_hz;
  return tstep;
}
...
int32_t Converter::accelerationChipToHzPerS(int32_t acceleration_chip) {
  ...
  acceleration_hz_per_s =
      ...
      ((int64_t)converter_parameters_.clock_frequency_mhz *
       (int64_t)converter_parameters_.clock_frequency_mhz * 1000);
