...
public:
  struct Callbacks {
    ...
    size_t (*write)(void *ctx, uint8_t b){nullptr};
    ...
    void (*set_tx_enable)(void *ctx, bool enable){nullptr};
  };
  ...
  void configure(Callbacks callbacks, Config config) {
  ...
  bool configured() const {
    return (callbacks_.available != nullptr) && (callbacks_.read != nullptr) &&
  ...
  bool resultReady() const { return state_ == State::Done; }

  bool done() const { return resultReady(); }

  UartError lastError() const { return last_error_; }

  // Start a register read transaction. Completion is reported via resultReady()
  // and takeReadResult().
  Result<void> startRead(uint8_t node_address, uint8_t register_address) {
    Result<void> r;
    if (!configured()) {
  ...
  Result<void> startWrite(uint8_t node_address, uint8_t register_address,
                          uint32_t data) {
    Result<void> r;
    if (!configured()) {
      r.error = UartError::NotInitialized;
...
private:
  ...
  static uint32_t elapsedUs_(uint32_t now_us, uint32_t start_us) {
    return now_us - start_us;
  }

  // Wrap-safe "now >= t" comparison.
  static bool timeAfterEq_(uint32_t now_us, uint32_t t_us) {
    return static_cast<int32_t>(now_us - t_us) >= 0;
  }

  static uint32_t computeTxTimeUs_(size_t bytes, uint32_t baud_rate) {
    // Estimate: 1 start + 8 data + 1 stop = 10 bits per byte.
    const uint64_t bits = static_cast<uint64_t>(bytes) * 10ULL;
    const uint64_t us =
        (bits * 1000000ULL + static_cast<uint64_t>(baud_rate) - 1ULL) /
  ...
  DrainResult drainRx_() {
    ...
    if ((drained >= config_.drain_limit) &&
        (callbacks_.available(callbacks_.ctx) > 0)) {
  ...
  void handleErrorOrRetry_(UartError e, uint32_t now_us) {
    last_error_ = e;
