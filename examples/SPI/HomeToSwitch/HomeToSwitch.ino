...
// 51200 microsteps per revolution / 6.283 radians per revolution ~= 8149
// microsteps per radian one "real unit" in this example is one radian of
// rotation
...
const auto switch_parameters_homing_to_switch =
    tmc51x0::SwitchParameters{}.withLeftStopEnabled(true);
...
void setup() {
  ...
  controller_parameters_chip = stepper.converter.controllerParametersRealToChip(
      controller_parameters_real);
  stepper.controller.setup(controller_parameters_chip);

  home_parameters_homing_to_switch_chip =
      stepper.converter.homeParametersRealToChip(
          home_parameters_homing_to_switch_real);
  home_parameters_homing_off_switch_chip =
      stepper.converter.homeParametersRealToChip(
          home_parameters_homing_off_switch_real);
  ...
  while (stepper.controller.stepAndDirectionMode()) {
    Serial.println("Step and Direction mode enabled so SPI/UART motion "
...
void loop() {
  ...
  stepper.beginHomeToSwitch(home_parameters_homing_to_switch_chip,
                            switch_parameters_homing_to_switch);
  ...
  stepper.beginHomeToSwitch(home_parameters_homing_off_switch_chip,
                            switch_parameters_homing_off_switch);
  while (not stepper.homed()) {
    int32_t actual_position_chip = stepper.controller.readActualPosition();
    int32_t actual_position_real =
        stepper.converter.positionChipToReal(actual_position_chip);
  ...
  int32_t target_position_chip =
      stepper.converter.positionRealToChip(MOVE_POSITION);
