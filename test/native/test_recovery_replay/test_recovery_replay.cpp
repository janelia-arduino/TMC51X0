...
static void
test_driver_reinitialize_replays_latest_high_level_writes (void)
{
  FakeInterface interface;
  Registers registers;
...
static void
test_driver_reinitialize_preserves_custom_automatic_current_control_shape (void)
{
  FakeInterface interface;
  Registers registers;
  initRegisters (registers, interface);
...
test_controller_reinitialize_replays_latest_configuration_writes (void)
{
  ...
  TEST_ASSERT_EQUAL_UINT32 (PositionMode, interface.register_image[Registers::RampmodeAddress]);
  TEST_ASSERT_EQUAL_UINT32 (12345UL, interface.register_image[Registers::VmaxAddress]);
  TEST_ASSERT_EQUAL_UINT32 (99UL, interface.register_image[Registers::TzerowaitAddress]);
  TEST_ASSERT_EQUAL_UINT32 (456UL, interface.register_image[Registers::VdcminAddress]);
...
static void
test_restore_switch_settings_updates_recovery_state (void)
{
  FakeInterface interface;
  Registers registers;
  initRegisters (registers, interface);
  ...
  SwitchParameters original = SwitchParameters{}
                                  .withLeftStopEnabled (true)
                                  .withLatchEncoderEnabled (true);
  SwitchParameters temporary = SwitchParameters{}
      ...
      char **argv)
{
  ...
  RUN_TEST (test_driver_reinitialize_preserves_custom_automatic_current_control_shape);
