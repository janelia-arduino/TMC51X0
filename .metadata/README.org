...
- Version :: 9.0.10
- License :: BSD
- URL :: https://github.com/janelia-arduino/TMC5130
- Author :: Peter Polidoro
- Email :: peter@polidoro.io

** Description

The TMC5130 is an ultra-silent motor driver IC for two phase stepper motors with
both UART serial and step and direction interfaces.

#+html: <img src="./images/trinamic_wiring-TMC5130-description.svg" width="1200px">
...
each other, as is the case with the TMC5130 stepper motor driver. One controller
...
** TMC5130 Stepper Motor Driver and Controller Combination

The TMC5130 is a stepper motor driver and it needs a stepper motor controller
communicating with it.

The TMC5130 can be used as both a stepper motor driver and stepper motor
...
velocity depends on the TMC5130 clock frequency. The TMC5130 clock frequency
...
TMC5130 and this library, for position and acceleration control.

*** Microcontroller Stepper Motor Controller

One controller option is to use just a single microcontroller, communicating
with the TMC5130 over both the UART serial interface and the step and direction
interface.

#+html: <img src="./images/trinamic_wiring-TMC5130-microcontroller.svg" width="1200px">
...
#+html: <img src="./images/trinamic_wiring-TMC5130-stepper-controller.svg" width="1200px">

* Communication

The TMC5130 driver has two interfaces to communicate with a stepper motor
...
The TMC5130 communicates over a UART serial port using a single wire interface,
...
TMC5130 parameters may be set using unidirectional communication from a
microcontroller UART serial TX pin to the TMC5130 PDN_UART pin. Responses from
the TMC5130 to the microcontroller are ignored.

#+html: <img src="./images/trinamic_wiring-TMC5130-unidirectional.svg" width="1200px">

*** Bidirectional Communication

The UART single wire interface allows control of the TMC5130 with any set of
microcontroller UART serial TX and RX pins.

**** Coupled

The simpliest way to connect the single TMC5130 serial signal to both the
...
TX commands from the microcontroller to the TMC5130 on the microcontroller RX
line. These echos need to be removed by this library in order to properly read
responses from the TMC5130.

Another disadvantage to coupling the TX and RX lines together is that it limits
the length of wire between the microcontroller and the TMC5130. The TMC5130
...
#+html: <img src="./images/trinamic_wiring-TMC5130-bidirectional-coupled.svg" width="1200px">

*** Serial Setup

The microcontroller serial port must be specified during the TMC5130 setup.
...
void setup()
{
  stepper_driver.setup(serial_stream, SERIAL_BAUD_RATE, TMC5130::SERIAL_ADDRESS_0, RX_PIN, TX_PIN);
}
#+END_SRC
...
and 1 cannot be used on these boards to communicate with the TMC5130. Connecting
...
"Serial2", can use those ports to communicate with the TMC5130.
...
0 and 1, allowing pins 0 and 1 to be used to communicate with a TMC5130 using
"Serial1".

*** Serial Baud Rate

The serial baud rate is the speed of communication in bits per second of the
UART serial port connected to the TMC5130.
...
required, as the TMC5130 automatically adapts to the baud rate. In practice, it
...
*** Connecting multiple TMC5130 chips

**** Unidirectional Communication

***** All chips using identical settings

If only unidirectional communication is desired and all TMC5130 chips connected
...
// Instantiate a single TMC5130 to talk to multiple chips
TMC5130 stepper_drivers;
...
#+html: <img src="./images/trinamic_wiring-TMC5130-unidirectional-multiple.svg" width="1200px">
...
// Instantiate the two TMC5130
TMC5130 stepper_driver_0;
const TMC5130::SerialAddress SERIAL_ADDRESS_0 = TMC5130::SERIAL_ADDRESS_0;
TMC5130 stepper_driver_1;
const TMC5130::SerialAddress SERIAL_ADDRESS_1 = TMC5130::SERIAL_ADDRESS_1;
const long SERIAL_BAUD_RATE = 115200;

void setup()
{
  // TMC5130::SERIAL_ADDRESS_0 is used by default if not specified
...
#+html: <img src="./images/trinamic_wiring-TMC5130-unidirectional-multiple-address.svg" width="1200px">
...
#+html: <img src="./images/trinamic_wiring-TMC5130-unidirectional-multiple-uart.svg" width="1200px">

**** Bidirectional Communication

***** Chips needing different settings using one UART

More than one TMC5130 may be connected to a single serial port, if each TMC5130
...
using the TMC5130 hardware input pins MS1 and MS2, to "SERIAL_ADDRESS_1",
"SERIAL_ADDRESS_2", or "SERIAL_ADDRESS_3".

The TMC5130 serial address must be specified during the TMC5130 setup, if it is
not equal to the default of "SERIAL_ADDRESS_0".

When multiple TMC5130 chips are connected to the same serial line with multiple
...
#+html: <img src="./images/trinamic_wiring-TMC5130-bidirectional-coupled-multiple-address.svg" width="1200px">
...
#+html: <img src="./images/trinamic_wiring-TMC5130-bidirectional-coupled-multiple-uart.svg" width="1200px">
...
for the TMC5130 chip during power up.
...
The TMC5130 chip has an enable input pin that switches off the power stage,
...
method to assign a microcontroller pin to contol the TMC5130 enable line.
...
The TMC5130 may also be enabled and disabled in software, independent of the
hardware enable pin.
...
The TMC5130 can operate in one of two modes, the voltage control mode and the
...
#+html: <img src="./images/trinamic_wiring-TMC5130-teensy40.svg" width="1200px">

*** Mega 2560

#+html: <img src="./images/trinamic_wiring-TMC5130-mega2560.svg" width="1200px">

*** Uno

#+html: <img src="./images/trinamic_wiring-TMC5130-uno.svg" width="1200px">
...
** TMC5130 Stepper Driver Integrated Circuit

[[https://www.trinamic.com/products/integrated-circuits/details/tmc5130-la][Trinamic TMC5130-LA Web Page]]
...
[[https://www.trinamic.com/support/eval-kits/details/silentstepstick][Trinamic TMC5130 SilentStepStick Web Page]]

** BIGTREETECH TMC5130 V1.2 UART Stepper Motor Driver

[[https://www.biqu.equipment/products/bigtreetech-tmc5130-stepper-motor-driver-for-3d-printer-board-vs-tmc2208][BIGTREETECH TMC5130 Web Page]]
