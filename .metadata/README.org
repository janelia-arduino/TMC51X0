...
#+html: <img src="./images/trinamic_wiring-TMC51X0-description.svg" width="1200px">
...
** TMC51X0 Stepper Motor Driver and Controller Combination

The TMC51X0 is a stepper motor driver and it needs a stepper motor controller
communicating with it.

The TMC51X0 can be used as both a stepper motor driver and stepper motor
...
velocity depends on the TMC51X0 clock frequency. The TMC51X0 clock frequency
...
TMC51X0 and this library, for position and acceleration control.

*** Microcontroller Stepper Motor Controller

One controller option is to use just a single microcontroller, communicating
with the TMC51X0 over both the UART serial interface and the step and direction
interface.

#+html: <img src="./images/trinamic_wiring-TMC51X0-microcontroller.svg" width="1200px">
...
#+html: <img src="./images/trinamic_wiring-TMC51X0-stepper-controller.svg" width="1200px">

* Communication

The TMC51X0 driver has two interfaces to communicate with a stepper motor
...
The TMC51X0 communicates over a UART serial port using a single wire interface,
...
TMC51X0 parameters may be set using unidirectional communication from a
microcontroller UART serial TX pin to the TMC51X0 PDN_UART pin. Responses from
the TMC51X0 to the microcontroller are ignored.

#+html: <img src="./images/trinamic_wiring-TMC51X0-unidirectional.svg" width="1200px">

*** Bidirectional Communication

The UART single wire interface allows control of the TMC51X0 with any set of
microcontroller UART serial TX and RX pins.

**** Coupled

The simpliest way to connect the single TMC51X0 serial signal to both the
...
TX commands from the microcontroller to the TMC51X0 on the microcontroller RX
line. These echos need to be removed by this library in order to properly read
responses from the TMC51X0.

Another disadvantage to coupling the TX and RX lines together is that it limits
the length of wire between the microcontroller and the TMC51X0. The TMC51X0
...
#+html: <img src="./images/trinamic_wiring-TMC51X0-bidirectional-coupled.svg" width="1200px">

*** Serial Setup

The microcontroller serial port must be specified during the TMC51X0 setup.
...
void setup()
{
  stepper_driver.setup(serial_stream, SERIAL_BAUD_RATE, TMC51X0::SERIAL_ADDRESS_0, RX_PIN, TX_PIN);
}
#+END_SRC
...
and 1 cannot be used on these boards to communicate with the TMC51X0. Connecting
...
"Serial2", can use those ports to communicate with the TMC51X0.
...
0 and 1, allowing pins 0 and 1 to be used to communicate with a TMC51X0 using
"Serial1".

*** Serial Baud Rate

The serial baud rate is the speed of communication in bits per second of the
UART serial port connected to the TMC51X0.
...
required, as the TMC51X0 automatically adapts to the baud rate. In practice, it
...
*** Connecting multiple TMC51X0 chips

**** Unidirectional Communication

***** All chips using identical settings

If only unidirectional communication is desired and all TMC51X0 chips connected
...
// Instantiate a single TMC51X0 to talk to multiple chips
TMC51X0 stepper_drivers;
...
#+html: <img src="./images/trinamic_wiring-TMC51X0-unidirectional-multiple.svg" width="1200px">
...
// Instantiate the two TMC51X0
TMC51X0 stepper_driver_0;
const TMC51X0::SerialAddress SERIAL_ADDRESS_0 = TMC51X0::SERIAL_ADDRESS_0;
TMC51X0 stepper_driver_1;
const TMC51X0::SerialAddress SERIAL_ADDRESS_1 = TMC51X0::SERIAL_ADDRESS_1;
const long SERIAL_BAUD_RATE = 115200;

void setup()
{
  // TMC51X0::SERIAL_ADDRESS_0 is used by default if not specified
...
#+html: <img src="./images/trinamic_wiring-TMC51X0-unidirectional-multiple-address.svg" width="1200px">
...
#+html: <img src="./images/trinamic_wiring-TMC51X0-unidirectional-multiple-uart.svg" width="1200px">

**** Bidirectional Communication

***** Chips needing different settings using one UART

More than one TMC51X0 may be connected to a single serial port, if each TMC51X0
...
using the TMC51X0 hardware input pins MS1 and MS2, to "SERIAL_ADDRESS_1",
"SERIAL_ADDRESS_2", or "SERIAL_ADDRESS_3".

The TMC51X0 serial address must be specified during the TMC51X0 setup, if it is
not equal to the default of "SERIAL_ADDRESS_0".

When multiple TMC51X0 chips are connected to the same serial line with multiple
...
#+html: <img src="./images/trinamic_wiring-TMC51X0-bidirectional-coupled-multiple-address.svg" width="1200px">
...
#+html: <img src="./images/trinamic_wiring-TMC51X0-bidirectional-coupled-multiple-uart.svg" width="1200px">
...
for the TMC51X0 chip during power up.
...
The TMC51X0 chip has an enable input pin that switches off the power stage,
...
method to assign a microcontroller pin to contol the TMC51X0 enable line.
...
The TMC51X0 may also be enabled and disabled in software, independent of the
hardware enable pin.
...
The TMC51X0 can operate in one of two modes, the voltage control mode and the
...
#+html: <img src="./images/trinamic_wiring-TMC51X0-teensy40.svg" width="1200px">

*** Mega 2560

#+html: <img src="./images/trinamic_wiring-TMC51X0-mega2560.svg" width="1200px">

*** Uno

#+html: <img src="./images/trinamic_wiring-TMC51X0-uno.svg" width="1200px">
...
** TMC51X0 Stepper Driver Integrated Circuit

[[https://www.trinamic.com/products/integrated-circuits/details/tmc5130-la][Trinamic TMC51X0-LA Web Page]]
...
[[https://www.trinamic.com/support/eval-kits/details/silentstepstick][Trinamic TMC51X0 SilentStepStick Web Page]]

** BIGTREETECH TMC51X0 V1.2 UART Stepper Motor Driver

[[https://www.biqu.equipment/products/bigtreetech-tmc5130-stepper-motor-driver-for-3d-printer-board-vs-tmc2208][BIGTREETECH TMC51X0 Web Page]]
